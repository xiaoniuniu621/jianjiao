<template>
	<div id="furniture"  v-if="getFurnitureList.length>0">
...
//			})
			...
			if(this.$store.state.FurnitureList.length==0){
				this.$store.dispatch("getFurnitureList")			
			}
		...
		computed:{
			getFurnitureList(){
				if(this.$store.state.FurnitureList.length>0){
					return this.$store.state.FurnitureList
				}else{
					return []
				}
			},
			carouselList(){				
					return this.$store.state.FurnitureList[0].moduleContent.banners
			},
			slideList(){
					return this.$store.state.FurnitureList[1]
			},
			brand(){
					return this.$store.state.FurnitureList[2]
			},
			classList(){
					return this.$store.state.FurnitureList[3]
			},
			columList(){
					return this.$store.state.FurnitureList.slice(4)
			},
			fixedList(){
					return this.$store.state.FurnitureList.slice(3)
			}
